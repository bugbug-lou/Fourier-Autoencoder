...
import math
import numpy as np
from tools import get_all_axis
...
class AE(object):
    def __init__(self, input_dim, hidden_dim, compress_dim, k, thres):
    ...
    def FP(self, dataset, compressed, k, thres):
        ## thres: the axis entry maximum value
        ## k: specifying the function
        ## get_all: function to be completed
        testfunc = torch.sin(torch.matmul(dataset, k))
        dim = compressed.shape[1]
        axes = get_all_axis(dim, thres)
        FP_loss = torch.tensor([0.0])
        for arr in axes:
            arr = torch.FloatTensor(arr)
            coeff = get_fc(compressed, testfunc, arr)
